...
message.textContent = `Ход: ${currentPlayer ? 'Крестика' : 'Нолика'}`
...
function placeSymbol(id) {
	...
	if (cell.textContent === '') {
		...
		if (checkWinner()) {
			gameOver = true
			message.textContent = `Победил ${currentPlayer ? 'Крестик' : 'Нолик'}`
		} else if (checkDraw()) {
			gameOver = true
			message.textContent = 'Ничья'
		} else {
			message.textContent = `Ход: ${currentPlayer ? 'Нолика' : 'Крестика'}`
